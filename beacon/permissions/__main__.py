...
from beacon.auth.__main__ import AuthN
...
class AuthZ(AuthN):
    ...
    async def authorization(self, request):
        try:
            ...
            user, list_visa_datasets = await self.authentication(self, access_token)
            if user is None:
                user = 'public'
            elif user == 'public':
                username = 'public'
    ...
    @log_with_args(level=level)
    def dataset_permissions(self, func):
        @log_with_args(level=level)
        async def permission(request: Request):
            ...
            username, list_visa_datasets = await self.authorization(request)
            LOG.debug(username)
