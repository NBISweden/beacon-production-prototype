...
from beacon.__main__ import EndpointView

class AuthN(EndpointView):
    ...
    async def authentication(self, access_token):
        ...
        try:
            ...
            access_token_validation = self.validate_access_token(self, access_token, idp_issuer, idp_jwks_url, algorithm, aud)
            if access_token_validation == True:
                user, list_visa_datasets = await self.fetch_user_info(self, access_token, user_info, idp_issuer, list_visa_datasets)
                return user, list_visa_datasets
        except Exception:
